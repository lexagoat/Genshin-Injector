...
bool injector::inject(const std::string& dll_path)
{
    ...
    if (!loader_memory)
    {
        printf("[!] failed to allocate memory for loader. GetLastError: 0x%1X\n",
            GetLastError());
        return false;
    }

    printf("[+] loader allocated @ 0x%11X\n", loader_memory);
    ...
    const HANDLE thread_handle = CreateMemoryThread(mem_utils::process_handle,
        NULL,
        0,
    ...
    {
        printf("[!] failed to create remote thread. GetLastError: 0x%1x\n",
            GetLastError());
        return false;
    }

    printf("[-] remote thread created (0x%X)\n", thread_handle);
    printf("[+] address of loader: 0x%11X\n[+] address of image: 0x%11X\n",
        loader_memory,
        executable_image);
